...
##### *军：
> 老师说前面看您有说测过百万级别项目，单机1000内线程的话，当时您用了多少节点机？或者说怎么处理的？ 希望能回一下

 ###### &nbsp;&nbsp;&nbsp; 讲师回复：
> &nbsp;&nbsp;&nbsp; 这个不同的服务器，甚至物理机上分布的虚机都会有影响，我这边最多用过30台机器分布式测过。

##### **华：
> JMETER基础、层次讲得非常好，以前自学了JMETER，层次不清楚，看了之后茅塞顿开

##### **臣：
> 写的很好，既精简，又透彻

##### **森：
> 老师，我想问下，如何可以判断出jmeter遇到了自身瓶颈，然后需要开始考虑分布式了呢？

 ###### &nbsp;&nbsp;&nbsp; 讲师回复：
> &nbsp;&nbsp;&nbsp; 1.监控，参加第三模块的方案
2.客户端响应时间慢，服务端链路监控很快，这是一个排查切入点
...
##### *强：
> 创建元件时，选择KeepAlive,可以复用线程。单主控机控制的数量控制在10个内。2.9后，剥离响应数据的结果发送到控制台，减少网络io的影响。定期清理日志文件。
...
##### *欢：
> 请问老师，一般我们说的并发数与jmeter中的线程数设置有关系吗？比如线程数设置为50，ramp-up时间设置为10，循环次数为永远。可以认为总会有某一时刻，并发数会达到50吗？

 ###### &nbsp;&nbsp;&nbsp; 讲师回复：
> &nbsp;&nbsp;&nbsp; 在实际应用场景下，不会把工具的线程数和实际并发数去类比，后面的我会讲核心的指标TPS。
