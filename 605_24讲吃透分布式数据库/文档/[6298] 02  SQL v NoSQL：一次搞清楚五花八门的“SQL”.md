<p data-nodeid="160555" class="">这一讲我们开始讨论有关 SQL 及其变种的前世今生，以及它与分布式数据库之间的纷繁复杂关系。</p>
...
<p data-nodeid="159879">探索事物的本质是一件很有趣的事情，希望你通过学习也能感受到这份乐趣，下一讲我们来聊聊分布式数据库的核心之一——数据分片。到时见。</p>
...
##### *伟：
> 我觉得InnoDB Cluster 和 MyCat 之类的一样，还是数据库中间件范畴。newSql 是 Rdbms 和 noSql 的结合。
...
##### **文：
> 我理解innodb cluster只能满足高可用 无法对数据进行分片存储 不是分布式数据库

##### **云：
> MySQL 8.0 引入的 InnoDB Cluster 应该是NewSql 类型吧，不能分布式处理

 ###### &nbsp;&nbsp;&nbsp; 讲师回复：
> &nbsp;&nbsp;&nbsp; 单独的InnoDB Cluster缺少sharding，但是我们可以引入sharding方案来打造为分布式数据库。

##### 薛：
> InnoDB Cluster属于NewSQL，对不对？
