...
##### **0220：
> 你好，权限控制模式总结一下就是两种类型四种模式，两种类型是范围和口令，模式是范围模式，口令模式，digest模式，world模式，可以这么理解吗

##### **天空：
> 打卡完成：ACL 权限控制

##### **名先生：
> 会话的权限信息是存储在zk的内存中的，宕机或者会话关闭会清空，再次启动需要重新调用接口进行授权。

##### **航：
> <span style="font-size: 16.7811px;">到目前为止我们对 ACL 权限在 ZooKeeper 服务器客户端和服务端的底层实现过程进行了深度的分析。总体来说，客户端在 ACL 权限请求发送过程的步骤比较简单：首先是封装该请求的类型，之后将权限信息封装到 request 中并发送给服务端。而服务器的实现比较复杂，首先分析请求类型是否是权限相关操作，之后根据不同的权限模式（scheme）调用不同的实现类验证权限最后存储权限信息。本课时的例子采用了授权接口 addAuth 而没有采用权限设置接口 setAcl，是因为权限设置接口相对简单，其核心功能点已经包括在授权接口实现中。而在授权接口中，值得注意的是会话的授权信息存储在 ZooKeeper 服务端的内存中，如果客户端会话关闭，授权信息会被删除。下次连接服务器后，需要重新调用授权接口进行授权。</span>

##### *文：
> world模式呢？auth模式呢？为什么有的先setAcl,有的先addAuth？

 ###### &nbsp;&nbsp;&nbsp; 讲师回复：
> &nbsp;&nbsp;&nbsp; setacl 是权限设置，相当于设置一个用户名密码的操作
addauth 是授权操作，相当于给某一个人或账号授予数据节点的相关操作权限
口令模式就是auth模式
